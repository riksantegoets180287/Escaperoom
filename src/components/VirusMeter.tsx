...
export function VirusMeter({ completedCount, codePieces, completedGames }: VirusMeterProps) {
      ...
      <div className="flex justify-center gap-4">
        {segments.map((s) => (
          <div key={s} className="flex flex-col items-center gap-2">
            ...
            >
              {/* Ticket look */}
              <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-white border border-gray-200" />
              <div className="absolute -right-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-white border border-gray-200" />
              
              <span className={`text-xl font-bold ${completedGames[s] ? 'text-[#20126E]' : 'text-gray-300'}`}>
                {completedGames[s] ? codePieces[s] : '?'}
              </span>
            </motion.div>
